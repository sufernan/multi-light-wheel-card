...
export class MultiLightWheelCard extends LitElement {
  ...
  @state() private selectionSource: "wheel" | "button" | null = null;
  ...
  private selectSingleEntityFromButton(entityId: string): void {
    ...
    const groupToExpand = this.getMarkerGroups().find(
      (group) => group.count > 1 && group.entityIds.includes(entityId)
    );
